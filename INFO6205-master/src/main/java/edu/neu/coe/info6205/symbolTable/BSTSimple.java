...
//    }
    ...
    Node root = null;//////////////////////////////////////////////////////////////
    //NodeValue result =new NodeValue(root,1);
    ...
    private NodeValue put(Node node, Key key, Value value) 
    {
        ...
        if (node == null) 
        	return new NodeValue(new Node(key, value), null);
        int cf = key.compareTo(node.key);
        if (cf == 0) 
        {
    ...
    private int largeDepth(Node root) {
        if (root == null) return 0;
        int left = largeDepth(root.smaller);
        int right = largeDepth(root.larger);
        return Math.max(left, right) + 1;
    }
    
    public int getlargeDepth() {
    	return largeDepth(root);
    }
